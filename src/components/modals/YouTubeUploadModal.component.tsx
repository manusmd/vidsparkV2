...
import { useChannelAnalytics } from "@/hooks/data/useChannelAnalytics";
...
export const YoutubeUploadModal: React.FC<YouTubeUploadModalProps> = ({
  ...
  // No longer need selectedDay state since we removed the day tabs
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[550px] h-[800px] overflow-y-auto scrollbar-thin scrollbar-thumb-rounded-full scrollbar-thumb-primary/20 hover:scrollbar-thumb-primary/30 scrollbar-track-transparent">
        <DialogHeader className="space-y-2">
          ...
          <DialogDescription>
            Schedule your video to be published on YouTube. Select a channel,
            date, time, and privacy setting.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 py-2">
          ...
          {/* Hidden analytics refresh button */}
          {channelId && (
            <div className="hidden">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 ${isLoadingAnalytics ? "animate-spin" : ""}`}
                />
              </Button>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <Tabs
              value={publishMode}
              onValueChange={(value) =>
                setPublishMode(value as "now" | "schedule")
              }
            >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Your video will be published as soon as processing is
                          complete
                        </p>
                      </div>
                    </div>
                  ...
                  <Card className="border border-input">
                    <CardContent>
                      <div className="flex flex-col gap-4">
                        ...
                        {analytics?.bestPostingTimes?.hours &&
                          analytics?.bestPostingTimes?.hours?.length > 0 && (
                              ...
                              <div className="flex items-center justify-between">
                                <div>
                                  ...
                                  <p className="text-sm mt-1">
                                    ...
                                    <span className="font-medium">
                                      {
                                        analytics?.bestPostingTimes?.hours?.[0]
                                          ?.hourFormatted
                                      }
                                    </span>
                                  </p>
                                  <p className="text-xs text-muted-foreground mt-1">
                                    Based on{" "}
                                    {analytics?.videoAnalysis &&
                                    analytics?.videoAnalysis?.videosAnalyzed > 0
                                      ? `${analytics?.videoAnalysis?.videosAnalyzed} videos from your channel`
                                      : "channel analytics"}
                                  </p>
                                </div>
                                <Button
                                  ...
                                  className="h-8"
                                  onClick={() => {
                                    // Set the date to the next occurrence of the best day
                                    const today = new Date();
                                    const bestDayNum = parseInt(
                                      analytics?.bestPostingTimes?.hours?.[0]
                                        ?.day || "3",
                                    );
                                    const todayNum = today.getDay();
                                    const daysToAdd =
                                      (bestDayNum + 7 - todayNum) % 7;
                                    const nextBestDay = new Date(today);
                                    nextBestDay.setDate(
                                      today.getDate() + daysToAdd,
                                    );

                                    // Set the time to the best hour
                                    const bestHourNum = parseInt(
                                      analytics?.bestPostingTimes?.hours?.[0]
                                        ?.hour || "15",
                                    );
                                    const bestHour = new Date();
                                    bestHour.setHours(bestHourNum, 30, 0, 0); // Set minutes to 30 instead of 0

                                    setPublishDate(nextBestDay);
                                    setPublishTime(bestHour);
                                  }}
                                >
                                  Use Best Time
                                </Button>
                        ...
                        <div className="flex flex-row gap-3">
                          <div
                            className={cn(
                              "relative rounded-lg border p-3 cursor-pointer transition-all flex-1",
                              publishDate
